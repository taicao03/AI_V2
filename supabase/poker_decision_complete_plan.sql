...
alter table public.poker_players
  add constraint poker_players_player_status_check
  check (player_status in ('folded', 'all-in', 'thinking', 'ready', 'waiting'));
...
begin
  ...
  if alive_count <= 1 then
    ...
    where p.table_id = round_record.table_id
      ...
      and p.is_spectator = false
      and p.has_folded = false
    order by p.seat_order asc nulls last
    limit 1;

    update public.poker_hands h
    set is_winner = coalesce(h.user_id = sole_winner, false),
        revealed_at = now()
    ...
    for hand_record in
      ...
      for update
    loop
      if array_length(hand_record.cards, 1) = 3 and array_length(round_record.community_cards, 1) = 2 then
      ...
      from public.poker_evaluate_hand(combined_cards)
      limit 1;
    ...
    update public.poker_hands h
    set is_winner = case
      when top_score is null then false
      else coalesce(h.hand_score = top_score, false)
    end
    from public.poker_players p
    where h.round_id = p_round_id
...
declare
  ...
  pot_pressure numeric := 0.0;
  bankroll_factor numeric := 1.0;
  allin_bias numeric := 0.0;
begin
  ...
  pot_pressure := (coalesce(round_record.pot_amount, 0)::numeric + to_call::numeric) / greatest(available::numeric, 1);
  bankroll_factor := least(2.2, greatest(0.8, available::numeric / greatest((table_record.min_bet * 40)::numeric, 1)));
  allin_bias := least(0.55, greatest(0.0, 0.08 + (bankroll_factor - 1.0) * 0.20 + (pot_pressure * 0.10)));
  ...
  if hand_rank <= 2 and random() < least(0.98, 0.70 * risk_mult + 0.20 + allin_bias) then
    action := 'all-in';
    raise_to := null;
    return next;
    return;
  ...
  if hand_rank = 3 and to_call >= greatest(1, floor(available * (0.36 / greatest(0.65, risk_mult)))::integer) and random() < least(0.82, 0.58 + allin_bias) then
    action := 'all-in';
    raise_to := null;
    return next;
    return;
  ...
  if raise_room >= step_raise and (
    ...
    target_raise := coalesce(round_record.current_bet, 0) + greatest(
      step_raise,
      floor(available * (0.18 * risk_mult + 0.06 * bankroll_factor + random() * 0.18))::integer
    );
  ...
  if hand_rank <= 4 and available > 0 and random() < least(0.70, allin_bias + 0.18) then
    action := 'all-in';
    raise_to := null;
    return next;
    return;
  ...
  if hand_rank <= 6 then
    if to_call <= greatest(1, floor(available * (0.72 * risk_mult + 0.08 * bankroll_factor))::integer) or random() < bluff_chance then
      action := 'call';
    else
      action := case when random() < least(0.50, 0.16 * risk_mult + allin_bias) then 'all-in' else 'fold' end;
    end if;
  elsif hand_rank = 7 then
    if to_call <= greatest(1, floor(available * (0.42 * risk_mult + 0.04 * bankroll_factor))::integer) or random() < (bluff_chance * 0.9) then
      action := 'call';
    else
      action := 'fold';
    end if;
...
declare
  ...
  npc_table_id uuid;
  npc_table_name text;
  npc_table_count integer := 0;
  npc_target_tables integer := 3;
  npc_seed_names text[] := array['NPC Arena Alpha', 'NPC Arena Bravo', 'NPC Arena Sigma', 'NPC Arena Omega'];
  npc_seed_idx integer := 1;
  wealthy_bot_count integer := 0;
  elite_bot_count integer := 0;
  npc_host_uid uuid;
  npc_min_bet integer := 10;
  npc_max_bet integer := 1000;
  ready_count integer;
...
begin
  ...
  where p.left_at is null
    and p.is_spectator = false
    and p.is_bot = false
    and coalesce(p.last_heartbeat_at, p.updated_at) < now() - interval '10 seconds';
  ...
  where p.is_bot = true
    and p.left_at is null
    and greatest(0, u.points - u.locked_points) >= 20000;

  select count(distinct p.user_id) into elite_bot_count
  from public.poker_players p
  join public.users u on u.uid = p.user_id
  where p.is_bot = true
    and p.left_at is null
    and greatest(0, u.points - u.locked_points) >= 100000;

  npc_target_tables := least(12, greatest(3, 3 + floor(wealthy_bot_count / 2.0)::integer + elite_bot_count));

  select count(*)
  into npc_table_count
  from public.poker_tables t
  where t.status <> 'closed'
    and t.is_private = false
    and t.name like 'NPC Arena %';

  while npc_table_count < npc_target_tables loop
    npc_table_name := npc_seed_names[((npc_seed_idx - 1) % array_length(npc_seed_names, 1)) + 1] || ' #' || npc_seed_idx::text;
    if npc_seed_idx % 4 = 1 then
      npc_min_bet := 10;
      npc_max_bet := 1000;
    elsif npc_seed_idx % 4 = 2 then
      npc_min_bet := 50;
      npc_max_bet := 5000;
    elsif npc_seed_idx % 4 = 3 then
      npc_min_bet := 200;
      npc_max_bet := 20000;
    else
      npc_min_bet := 1000;
    ...
    npc_seed_idx := npc_seed_idx + 1;
    npc_table_count := npc_table_count + 1;
    ...
    where p.is_bot = true
      and p.left_at is null
      and greatest(0, u.points - u.locked_points) >= npc_min_bet * 10
    order by greatest(0, u.points - u.locked_points) desc
    limit 1;
    ...
    values (
      npc_table_name, false, 6, npc_min_bet, npc_max_bet, 'waiting', coalesce(npc_host_uid, current_uid), now()
    )
  ...
  loop
    if table_record.status in ('waiting', 'countdown') then
            ...
            least(
              ...
              greatest(
                ...
                case coalesce(
                  ...
                  when 'aggressive' then greatest(
                    table_record.min_bet + floor((table_record.max_bet - table_record.min_bet) * (0.45 + random() * 0.55))::integer,
                    floor(greatest(0, u.points - u.locked_points) * (0.10 + random() * 0.22))::integer
                  )
                  when 'conservative' then greatest(
                    table_record.min_bet + floor((table_record.max_bet - table_record.min_bet) * (0.02 + random() * 0.22))::integer,
                    floor(greatest(0, u.points - u.locked_points) * (0.02 + random() * 0.08))::integer
                  )
                  when 'random' then greatest(
                    table_record.min_bet + floor((table_record.max_bet - table_record.min_bet) * random())::integer,
                    floor(greatest(0, u.points - u.locked_points) * random() * 0.18)::integer
                  )
                  else greatest(
                    table_record.min_bet + floor((table_record.max_bet - table_record.min_bet) * (0.18 + random() * 0.50))::integer,
                    floor(greatest(0, u.points - u.locked_points) * (0.05 + random() * 0.15))::integer
                  )
                end
              )
            )
      ...
      if table_record.name like 'NPC Arena %' then
        target_active_count := least(table_record.max_players, 6);
      elsif human_seated_count > 0 then
        target_active_count := least(table_record.max_players, 4);
      else
        target_active_count := 2;
      end if;

      if active_seated_count < target_active_count then
        bots_needed := greatest(0, least(table_record.max_players - seated_count, target_active_count - active_seated_count));
      ...
      -- Keep bot-only public tables alive so NPCs can self-host and gather naturally.
    end if;
    ...
    if table_record.status = 'waiting' then
      ...
      if table_record.countdown_ends_at is not null and table_record.countdown_ends_at <= now() then
        ...
        set has_folded = false,
            ...
            last_action = null,
            acted_in_phase = false,
            player_status = case when p.is_ready then 'ready' else 'waiting' end,
  ...
  return jsonb_build_object(
    ...
    'players', (
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'is_host', p.user_id = target_table.created_by,
        'points', u.points,
...
create or replace function public.poker_host_kick_player(
  p_session_token text,
  p_table_id uuid,
...
declare
  ...
  profile public.users%rowtype;
  target_table public.poker_tables%rowtype;
  host_player public.poker_players%rowtype;
  kicked_player public.poker_players%rowtype;
begin
  select * into profile from public.users where uid = current_uid;
  if not found or profile.is_banned then
    raise exception 'Account cannot manage poker table' using errcode = '42501';
  end if;
  ...
  if p_target_user_id = current_uid then
    raise exception 'Host cannot kick yourself' using errcode = '22023';
  end if;

  select * into host_player
  from public.poker_players p
  where p.table_id = p_table_id
    and p.user_id = current_uid
    and p.left_at is null
  ...
  if not found then
    raise exception 'Host seat not found' using errcode = 'P0002';
  end if;

  if target_table.created_by is null then
    update public.poker_tables
    set created_by = current_uid,
        updated_at = now(),
        last_activity_at = now()
    where table_id = p_table_id
  ...
  if target_table.created_by <> current_uid then
    raise exception 'Only room host can kick players' using errcode = '42501';
  end if;
  ...
  where p.table_id = p_table_id
    ...
    and p.left_at is null
  returning * into kicked_player;

  if not found then
    raise exception 'Target player not found in room' using errcode = 'P0002';
  end if;
...
revoke all on function public.poker_host_kick_player(text, uuid, uuid) from public, anon, authenticated;
grant execute on function public.poker_host_kick_player(text, uuid, uuid) to anon, authenticated;
